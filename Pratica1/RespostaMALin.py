...
def din_pend(y, t, u, u_ref, t_ref):
...
for k in range(kend-1):
    ...
    if k*Ta > 1:
        theta_ref[k] = 89*deg2rad
        u_ref[k] = p*l1*sin(theta_ref[k])/l2
        u[k] = u_ref[k]
    ...
    sol = odeint(din_pend, x0, [0.0, Ta], args=(u[k], u_ref[k], theta_ref[k]))
    theta[k + 1] = sol[:, 0][-1]
    thetap[k+1] = sol[:, 1][-1]
